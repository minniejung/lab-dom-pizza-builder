...
function renderButtons() {
  ...
  for (let key in state) {
    allBtn.forEach(el => el.addEventListener("click", (event => {
      if (state[key] === true) event.target.classList.toggle("active")
    }))) 
  }
}
...
function renderPrice() {
  ...
  const emptyUl = price.children[2];
  emptyUl.innerHTML = '';
  price.children[3].innerHTML = basePrice;

  let total = basePrice;

  for (let key in state) {
    if (state[key] === true && Object.keys(ingredients).includes(key)) {
      let newLi = document.createElement('li')
      newLi.innerText = `$${ingredients[key].price} ${ingredients[key].name}`
      emptyUl.appendChild(newLi);
      total += ingredients[key].price;
      price.children[3].innerHTML = total;
    }  
  }
